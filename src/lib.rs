...
mod tests {
    ...
    fn generate_series<T: FromPrimitive>() -> Vec<T> {
        [
            5.0_f32, 9.0, 2.0, 9.0, 0.0, 6.0, 3.0, 8.0, 5.0, 8.0,
            7.0, 8.0, 8.0, 0.0, 2.0, 5.0, 0.0, 5.0, 6.0, 7.0,
            3.0, 6.0, 1.0, 4.0, 4.0, 4.0, 3.0, 7.0, 5.0, 8.0
        ].iter().map(|&x| T::from_f32(x).unwrap()).collect()
    }
    ...
    macro_rules! test_for_type {
        ($ty: ident) => {
            mod $ty {
                ...
                #[test]
                fn test_robust() {
                    let result = crate::params().robust(true).fit(&generate_series::<$ty>(), 7).unwrap();
                ...
                fn test_bad_seasonal_degree() {
                    let result = crate::params().seasonal_degree(2).fit(&generate_series::<$ty>(), 7);
                    assert_eq!(
                        result.unwrap_err(),
